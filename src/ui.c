...
void ui_draw_list(file_list_t *list, int selection, int scroll_offset) {
    ...
    for (int i = 0; i < MAX_VISIBLE_ROWS; i++) {
        ...
        // Construct string: [DIR] Name  or      Name
        char line[64];
        if (entry->is_dir) {
            snprintf(line, sizeof(line), "/ %-30s", entry->name);
        } else {
            snprintf(line, sizeof(line), "  %-30s", entry->name);
        }
    ...
    // 3. Draw Footer (Instructions)
    int footer_y = 29; // Approx bottom
    
    // Fill footer
    nio_vram_fill(0, footer_y * 8, 320, 8, NIO_COLOR_GRAY);
    nio_vram_grid_puts(0, 0, 0, footer_y, "ARROWS: Nav | ENTER: Open | Q: Exit", NIO_COLOR_GRAY, NIO_COLOR_BLACK);
